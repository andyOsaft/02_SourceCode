 ...
 * Example usage:
 ...
 struct AMessage *pxMessage;
	...
	if( xQueue1 != 0 )
	{
		...
		if( xQueueSendToFront( xQueue1, ( void * ) &ulVar, ( uint32_t ) 10 ) != pdPASS )
		{
	...
	if( xQueue2 != 0 )
	{
		...
		pxMessage = & xMessage;
		xQueueSendToFront( xQueue2, ( void * ) &pxMessage, ( uint32_t ) 0 );
	}
 ...
 * Example usage:
 ...
 struct AMessage *pxMessage;
	...
	if( xQueue1 != 0 )
	{
		// Send an unsigned long.  Wait for 10 ticks for space to become
		// available if necessary.
		if( xQueueSendToBack( xQueue1, ( void * ) &ulVar, ( uint32_t ) 10 ) != pdPASS )
		{
			// Failed to post the message, even after 10 ticks.
		}
	}
	...
	if( xQueue2 != 0 )
	{
		...
		// queue is already full.
		pxMessage = & xMessage;
		xQueueSendToBack( xQueue2, ( void * ) &pxMessage, ( uint32_t ) 0 );
	}
 ...
 * Example usage:
 ...
 struct AMessage *pxRxedMessage;
	...
	if( xQueue != 0 )
	{
		...
		if( xQueuePeek( xQueue, &( pxRxedMessage ), ( uint32_t ) 10 ) )
		{
 ...
 * Example usage:
 ...
 struct AMessage *pxRxedMessage;
	...
	if( xQueue != 0 )
	{
		...
		// message is not immediately available.
		if( xQueueReceive( xQueue, &( pxRxedMessage ), ( uint32_t ) 10 ) )
		{
			// pcRxedMessage now points to the struct AMessage variable posted
			// by vATask.
		}
 ...
 * \ingroup QueueManagement
 */
signed portBASE_TYPE xQueueGenericReceive( xQueueHandle xQueue, void * const pvBuffer, uint32_t xTicksToWait, portBASE_TYPE xJustPeek );
 ...
 * Example usage:
 ...
 {
 char cValueToPost;
 const uint32_t xBlockTime = ( uint32_t )0xff;
 ...
 */
signed portBASE_TYPE xQueueAltGenericSend( xQueueHandle xQueue, const void * const pvItemToQueue, uint32_t xTicksToWait, portBASE_TYPE xCopyPosition );
signed portBASE_TYPE xQueueAltGenericReceive( xQueueHandle xQueue, void * const pvBuffer, uint32_t xTicksToWait, portBASE_TYPE xJustPeeking );
...
signed portBASE_TYPE xQueueCRSend( xQueueHandle xQueue, const void *pvItemToQueue, uint32_t xTicksToWait );
signed portBASE_TYPE xQueueCRReceive( xQueueHandle xQueue, void *pvBuffer, uint32_t xTicksToWait );
 ...
 */
portBASE_TYPE xQueueTakeMutexRecursive( xQueueHandle xMutex, uint32_t xBlockTime );
 ...
 */
xQueueSetMemberHandle xQueueSelectFromSet( xQueueSetHandle xQueueSet, uint32_t xBlockTimeTicks );
...
void vQueueWaitForMessageRestricted( xQueueHandle xQueue, uint32_t xTicksToWait );
