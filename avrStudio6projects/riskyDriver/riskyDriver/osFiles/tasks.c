...
PRIVILEGED_DATA static volatile uint16_t xTickCount 						= ( uint16_t ) 0U;
...
PRIVILEGED_DATA static volatile uint16_t xNextTaskUnblockTime				= ( uint16_t ) portMAX_DELAY;
 ...
 */
static void prvAddCurrentTaskToDelayedList( uint16_t xTimeToWake ) PRIVILEGED_FUNCTION;
 ...
 */
	...
	static uint16_t prvGetExpectedIdleTime( void ) PRIVILEGED_FUNCTION;
	...
	portBASE_TYPE xYieldRequired = pdFALSE;
		...
		taskENTER_CRITICAL();
		{
			...
			if( uxCurrentPriority != uxNewPriority )
			{
				...
				listSET_LIST_ITEM_VALUE( &( pxTCB->xEventListItem ), ( configMAX_PRIORITIES - ( uint16_t ) uxNewPriority ) );
...
/*----------------------------------------------------------*/
	...
	static uint16_t prvGetExpectedIdleTime( void )
	{
	uint16_t xReturn;
...
1. */
	...
	void vTaskStepTick( uint16_t xTicksToJump )
	{
...
/*-----------------------------------------------------------*/
	...
	void vTaskPlaceOnEventListRestricted( const xList * const pxEventList, uint16_t xTicksToWait )
	{
	uint16_t xTimeToWake;
	...
	tskTCB * const pxTCB = ( tskTCB * ) pxMutexHolder;
		...
		if( pxMutexHolder != NULL )
		{
			if( pxTCB->uxPriority < pxCurrentTCB->uxPriority )
			{
				...
				listSET_LIST_ITEM_VALUE( &( pxTCB->xEventListItem ), configMAX_PRIORITIES - ( uint16_t ) pxCurrentTCB->uxPriority );
	...
	tskTCB * const pxTCB = ( tskTCB * ) pxMutexHolder;
		...
		if( pxMutexHolder != NULL )
		{
			if( pxTCB->uxPriority != pxTCB->uxBasePriority )
			{
				...
				listSET_LIST_ITEM_VALUE( &( pxTCB->xEventListItem ), configMAX_PRIORITIES - ( uint16_t ) pxTCB->uxPriority );
				prvAddTaskToReadyQueue( pxTCB );
			}
		}
	}
