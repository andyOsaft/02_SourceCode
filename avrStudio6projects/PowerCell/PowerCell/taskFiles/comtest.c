...
#define comTX_MAX_BLOCK_TIME		( ( uint32_t ) 0x96 )
#define comTX_MIN_BLOCK_TIME		( ( uint32_t ) 0x32 )
#define comOFFSET_TIME				( ( uint32_t ) 3 )

/* We should find that each character can be queued for Tx immediately and we
don't have to block to send. */
#define comNO_BLOCK					( ( uint32_t ) 0 )

/* The Rx task will block on the Rx queue for a long period. */
#define comRX_BLOCK_TIME			( ( uint32_t ) 0xffff )
